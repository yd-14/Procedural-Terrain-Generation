...
void windowCloseCallback(GLFWwindow* wind)
{
...
void imGuiRenderer()
{
    ...
    if (ImGui::Button("Introduction", ImVec2(buttonWidth, buttonHeight)))
    {
        intro=true;
        instruct=false;
        credits=false;
    ...
    if(intro)
    {
        ImGui::Text("Welcome to the world of procedural terrain generation."); 
        ImGui::Text(" ");
        ImGui::Text("In this project, we will explore the fascinating realm of creating virtual landscapes ");
        ImGui::Text("algorithmically. By harnessing the power of GLFW and GLAD libraries, we will embark on a");
        ImGui::Text("journey to generate intricate terrains filled with mountains, valleys, rivers, and other");
        ImGui::Text("natural elements. Through a combination of procedural algorithms and OpenGL rendering, we");
        ImGui::Text("will bring to life immersive and visually stunning virtual environments that can be"); 
        ImGui::Text("explored and admired.");
        ImGui::Text(" ");
        ImGui::Text("So buckle up, and let's dive into the exciting world of procedural terrain generation!");
    }
    if(instruct)
    {
        ImGui::Text("Click on start button below to start the procedural terrain generation");
        ImGui::Text("Q button to exit from terrain display window");
        ImGui::Text("Use the mouse to rotate the camera");
        ImGui::Text("Use mouse scroll to zoom the camera");
        ImGui::Text("Use W, S, A, D keys to move around");
    }
    if(credits)
    {
        width=0.0f;
        width += ImGui::CalcTextSize("Credits").x;
        AlignForWidth(width);
        ImGui::Text("Credits:");
        width=0.0f;
        width += ImGui::CalcTextSize("Yugal Deep Singh (20bcs140)").x;
        AlignForWidth(width);
        ImGui::Text("Yugal Deep Singh (20bcs140)");
        width=0.0f;
        width += ImGui::CalcTextSize("Ashwani Kumar (20bcs023)").x;
        AlignForWidth(width);
        ImGui::Text("Ashwani Kumar (20bcs023)");
        width=0.0f;
        width += ImGui::CalcTextSize("Harshith R N (20bcs056)").x;
        AlignForWidth(width);
        ImGui::Text("Harshith R N (20bcs056)");
        width=0.0f;
        width += ImGui::CalcTextSize("Sunil Kumar Patidar (20bcs131)").x;
        AlignForWidth(width);
        ImGui::Text("Sunil Kumar Patidar (20bcs131)");
    }
    ...
    float availy = ImGui::GetContentRegionMax().y;
    ImGui::SetCursorPosY(availy - 10.0f - buttonHeight);
...
int main() {
    ...
    // std::cout << "Current Working Directory: " << getCurrentDirectoryOnWindows() << std::endl;
    ...
    GLFWwindow* win= glfwCreateWindow(800, 500, "Terrain Generation project", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        ImGui::Begin("Subwindow", nullptr, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
...
void load_model(GLuint &VAO, std::string filename) {
    ...
    tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, filename.c_str(), "obj");
